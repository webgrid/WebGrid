...
function ShowColors() {
	...
	for (var i=0; i<webColors.length; i++) {
		html += '<td bgcolor="' + webColors[i] + '">'
			+ '<a onclick="return SelectColor();" onfocus="showColor(\'' + webColors[i] +  '\');" onmouseover="showColor(\'' + webColors[i] +  '\');">'
			+ '<img border="0" src="images/spacer.gif" width="10" height="10" title="' + webColors[i] +  '" alt="' + webColors[i] +  '" /></a></td>';
		if ((i+1) % 18 == 0)
			html += '</tr><tr>';
	}
	html += '<tr><td colspan="18">'
		...
		+ '<tr><td>'
		+ '<img id="selectedColor" style="background-color:' + '121212' //tinyMCE.getWindowArg('input_color') 
		+ '" border="0" src="images/spacer.gif" width="80" height="16" />'
		+ '</td><td align="right">'
